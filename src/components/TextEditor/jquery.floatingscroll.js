...
let floatingScrollProto = {
  ...
  initWidget() {
    ...
    let widget = (instance.widget = $(
      `<div class="fl-scrolls" data-orientation="${ORIENTATION}"></div>`
    ));
  ...
  checkVisibility() {
    ...
    if (!mustHide) {
      ...
      mustHide =
        containerRect[X_END] <= maxVisibleCrossEnd ||
        containerRect[X_START] > maxVisibleCrossEnd;
    }
...
$.fn.floatingScroll = function (method = 'init', options = {}) {
  if (method === 'init') {
    ...
    if (orientation !== HORIZONTAL && orientation !== VERTICAL) {
      throw new Error(
        `Scrollbar orientation should be either “${HORIZONTAL}” or “${VERTICAL}”`
      );
    }
    this.each((index, el) =>
      Object.create(floatingScrollProto).init($(el), orientation)
    );
  } else if (
    Object.prototype.hasOwnProperty.call(floatingScrollProto, `${method}API`)
  ) {
